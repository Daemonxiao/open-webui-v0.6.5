    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-Internal-User-Name"] = user.name
                headers["X-Internal-User-Id"] = user.id
                headers["X-Internal-User-Email"] = user.email
                headers["X-Internal-User-Role"] = user.role
